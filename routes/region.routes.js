...
router.get('/getlist', function (req, res) {
        RegionModel.find({}, function (err, Data) {
            if (err) return res.status(500).send("There was a problem finding the data.");
            res.status(200).send(Data);
        });
});
router.get('/view/:id', function (req, res) {
      RegionModel.findById(req.params.id, function (err, Data) {
          if (err) return res.status(500).send("There was a problem finding the Data.");
...
router.put('/editData/:id', function (req, res) {
      RegionModel.findByIdAndUpdate(req.params.id, req.body, {new: true}, function (err, Data) {
...
router.delete('/delete/:id', function (req, res) {
    RegionModel.findByIdAndRemove(req.params.id, function (err, Data) {
